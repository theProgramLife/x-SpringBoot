...
public class SysUserController extends AbstractController {
	...
	public R list(@RequestParam Map<String, Object> params,
		...
		//只有超级管理员，才能查看所有管理员列表
		if(getUserId().compareTo(new BigDecimal(Constant.SUPER_ADMIN))!=0){
	...
	public R info(@PathVariable("userId") BigDecimal userId){
		SysUserEntity user = sysUserService.queryObject(userId);
		
		//获取用户所属的角色列表
		List<BigDecimal> roleIdList = sysUserRoleService.queryRoleIdList(userId);
	...
	public R delete(@RequestBody BigDecimal[] userIds){
		if(ArrayUtils.contains(userIds, new BigDecimal("1"))){
